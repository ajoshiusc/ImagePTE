...
print('AUC after CV:', np.mean(auc), np.std(auc))
...
for n_features in range(1,150):
    auc = cross_val_score(
        clf, X[:, ind_feat[:n_features]], y, cv=36, scoring=my_metric)
    print('AUC after feature selection: %d |%g(%g)' %
          (n_features, np.mean(auc), np.std(auc)))

ind_feat = np.argsort(-feature_importance)
print(lab_ids[ind[0][ind_feat]], lab_ids[ind[1]
                                         [ind_feat]], feature_importance[ind_feat])
