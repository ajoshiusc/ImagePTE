...
def read_data(study_dir,
    ...
    for subj in subids:
        ...
        mask_cer=os.path.join(study_dir, subj, 'BrainSuite/T1mni.cerebrum.mask.nii.gz')
        ...
        mask_cer_file=nl.image.load_img(mask_cer).get_data()
        t1_msk[t1_msk!=0]=1
        t1_msk[mask_cer_file==0]=0
...
def read_data_test(study_dir,
    ...
    for subj in subids:
        ...
        t2_file = os.path.join(study_dir, subj, 'T2mni.nii.gz')
        flair_file = os.path.join(study_dir, subj, 'FLAIRmni.nii.gz')
        seg = os.path.join(study_dir, subj, '%s.lesion.mask.nii.gz' %subj)
        mask_cer=os.path.join(study_dir, subj, 'BrainSuite/T1mni.cerebrum.mask.nii.gz')
