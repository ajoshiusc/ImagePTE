...
epi_measures = np.concatenate(
    (epi_lesion_vols,epi_connectivity), axis=1)
...
nonepi_measures = np.concatenate(
    (nonepi_lesion_vols,nonepi_connectivity), axis=1)
...
for current_gamma in gamma_range:
    pipe = Pipeline([('pca_apply', PCA(n_components=best_com, whiten=WHITEN)),
...
for nf in range(1, max_component):
    pipe = Pipeline([('pca_apply', PCA(n_components=nf, whiten=WHITEN)),
