...
parser.add_argument('--ind_rmv', type=list, default=[], help='')
parser.add_argument('--use_all', type=bool, default=False, help='')
ind_rmv = [0, 3, 4, 13, 15]

def remove_rois(data, ind_rmv):
    tmp = np.delete(data, ind_rmv, axis=1)
    data = np.delete(tmp, ind_rmv, axis=0)
    return data
...
if len(args.ind_rmv) > 0:
    conn_pte = remove_rois(conn_pte, ind_rmv)
...
if args.use_all == True:
    ...
    (.3*epi_lesion_vols,epi_connectivity), axis=1)
else:
...
if len(args.ind_rmv) > 0:
    conn_nonpte = remove_rois(conn_nonpte, ind_rmv)
...
if args.use_all == True:
    ...
    (.3*nonepi_lesion_vols,nonepi_connectivity), axis=1)
else:
