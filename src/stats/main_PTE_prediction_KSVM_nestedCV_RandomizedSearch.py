...
param_grid = {"pca__n_components": [37, 27, 17, 57], "svc__C": [0.0001, 0.001, 0.01, .1, .3, .6, 0.7, 0.9, 1, 1.5, 2, 3, 4, 5, 6, 7, 9, 10, 100], "svc__gamma": [
    1, 0.001, 0.05, 0.075, .1, .13, .15, .17, 0.2, 0.3, .5, 1, 5, 10, 100]}

param_grid = {"pca__n_components": [55,57,59], "svc__C": [.1, 1, 10, 100], "svc__gamma": [1, 0.001, 0.05, 0.075, .1, .13, .15, .17, 0.2, 0.3, .5, 1, 5, 10, 100]}
...
for i in tqdm(range(NUM_TRIALS)):
    ...
    # , random_state=1211)
    inner_cv = StratifiedKFold(n_splits=35)#, shuffle=False)
    outer_cv = StratifiedKFold(n_splits=36)#, shuffle=False)#, random_state=1211)

    clf = RandomizedSearchCV(
        estimator=pipe, param_distributions=param_grid, cv=inner_cv, scoring='roc_auc', n_iter=10)
