...
epi_measures = np.concatenate(
    (a['epi_lesion_vols'], a['epi_connectivity'], a['ALFF_pte']), axis=1)
nonepi_measures = np.concatenate(
    (a['nonepi_lesion_vols'], a['nonepi_connectivity'], a['ALFF_nonpte']), axis=1)
...
param_grid = {"pca__n_components": range(1, max_component,10), "svc__C": [0.0001, 0.01, .1, 0.7, 2,  7, 10, 100], "svc__gamma": [
    1, 0.001, .1, 10]}
...
for i in tqdm(range(NUM_TRIALS)):
    ...
    nested_score = cross_val_score(
        clf, X=X, y=y.T, cv=outer_cv, error_score='raise')
