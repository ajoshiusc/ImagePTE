...
for current_gamma in gamma_range:
    ...
    pipe = imbpipeline(steps = [['smote', SMOTE()], ['pca_apply', PCA(n_components=best_com, whiten=True)], ['svc', SVC(kernel='rbf',C=best_C, gamma=current_gamma, class_weight=args.svm_w, tol=1e-9)]])
...
for current_C in C_range:
    ...
    pipe = imbpipeline(steps = [['smote', SMOTE()], ['pca_apply', PCA(n_components=best_com, whiten=True)], ['svc', SVC(kernel='rbf',C=current_C, gamma=best_gamma, class_weight=args.svm_w,tol=1e-9)]])
    #auc = cross_val_score(clf, X, y, cv=37, scoring=my_metric)
    kfold = StratifiedKFold(n_splits=args.num_cv, shuffle=True,random_state=1211)
    auc = cross_val_score(pipe, X, y, cv=kfold, scoring=my_metric)
    #print('AUC on testing data:gamma=%g, auc=%g' % (current_gamma, np.mean(auc)))
...
for nf in range(1, max_component):
    ...
    pipe = imbpipeline(steps = [['smote', SMOTE()], ['pca_apply', PCA(n_components=nf, whiten=True)], ['svc', SVC(kernel='rbf', C=best_C,gamma=best_gamma, class_weight=args.svm_w,tol=1e-9)]])
...
# y = np.random.permutation(y)
    # pipe = Pipeline([('pca_apply', PCA(n_components=best_com, whiten=True)),
    #                 ('svc', SVC(kernel='rbf',C=best_C, gamma=best_gamma, class_weight=args.svm_w,tol=1e-9))])
    pipe = imbpipeline(steps = [['smote', SMOTE()], ['pca_apply', PCA(n_components=best_com, whiten=True)],
                    ['svc', SVC(kernel='rbf',C=best_C, gamma=best_gamma, class_weight=args.svm_w,tol=1e-9)]])
...
# y = np.random.permutation(y)
    ...
    pipe = imbpipeline(steps = [['smote', SMOTE()],['svc', SVC(kernel='rbf', C=best_C, gamma=best_gamma, class_weight=args.svm_w, tol=1e-9)]])
