...
def kernel_SVM_choose_gamma(bfp_path,
                      sub_files,
                      group_id,
                      nperm=1000,
                      len_time=235,
    ...
    kr = SVC(kernel='precomputed') #, alpha=1.1)
    ...
    for i, gamma in enumerate(gamma_values):
        for v in roi_ind[::5]: #range(0,num_vert,100):
            ...
            kr = SVC(kernel='precomputed')
            kr.fit(D_train, group_id[:num_sub-num_sub_val])
