...
def main():
    ...
    for subid in subIds.index[2:]:
        ...
        if os.path.isfile(fnamet1):
            ...
            plotting.plot_anat(imgt1, title="plot_anat", cut_coords=(0,0,0))

        if os.path.isfile(fnamet2):
            imgt2 = ni.load_img(fnamet2)
            plotting.plot_anat(imgt2, title="plot_anat", cut_coords=(0,0,0))

        if os.path.isfile(fnameflair):
            imgflair = ni.load_img(fnameflair)
            plotting.plot_anat(imgflair, title="plot_anat", cut_coords=(0,0,0))
