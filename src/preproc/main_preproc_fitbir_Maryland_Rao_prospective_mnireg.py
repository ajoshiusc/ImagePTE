...
def regparfun(subid):
    ...
    os.system('/home/ajoshi/BrainSuite18a/bin/bse -i ' + t1 + ' -o ' + t1bse + ' --auto --trim -p')

    os.system('flirt -in ' + t1bse + ' -nosearch -out ' + t1mni + ' -ref ${FSLDIR}/data/standard/MNI152_T1_1mm -omat ' + t1mnimat + ' -dof 6')

    t1 = os.path.join(subdir, 'T1r.nii.gz')
    t2mni = os.path.join(subdir, 'T1mni.nii.gz')
    if os.path.isfile(t1):
        # Apply the same transform (T1->MNI) to registered T2 to take it to mni space
        os.system('flirt -in ' + t1 + ' -ref ' + t1mni +
