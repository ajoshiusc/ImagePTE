...
#     gl = 171 // args.ngroup
#     slide_len = gl // 4
#     args.ngroup_olp = (171 - gl) // slide_len + 1 + 1
...
#         q = 0
#         for j in range(0, 171-gl, slide_len):
...
#             k = k + 1
#
#         features_all[k, :, :] = feature[i, :, total_len-gl:]
#         adj_all[k, :, :] = adj[i + q * gap, :, :]
#         labels_all[k] = labels[i + q * gap]
#     print(features_all.shape, args.ngroup_olp)
...
def cross_validation():
    ...
    epidata = np.load("/home/wenhuicu/ImagePTE/pte_gcn/PRGNN/ADHD_parPearson_BCI-DNI.npz")
    ...
    nonepidata = np.load("/home/wenhuicu/ImagePTE/pte_gcn/PRGNN/TDC_parPearson_BCI-DNI.npz")
    ...
    model = GCN(nfeat=features.shape[2] // args.ngroup,
                nhid=[200, 200, 50],
                # nhid=[200, 200, 100, 50],
    ...
    for epoch in range(args.epochs):
        train(epoch, model, optimizer, scheduler,
              train_features, train_adj, train_labels)
    ...
    torch.save({'epoch': args.epochs,
        ...
        }, args.model_path+"model_ADHD_pret_dc_ovlp_split2.pth")
    input("any key")
