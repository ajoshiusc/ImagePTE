...
from utils import accuracy, accuracy_dc, find_common_graph, get_graphs_from_mask
...
def calc_DAD(data):
    thr = 0.4
    adj = np.abs(data['conn_mat'])
    mask = find_common_graph(adj, thr, 16)
...
def train(epoch, model, optimizer, scheduler, train_features, train_adj, train_labels):
    ...
    batch_size = args.ngroup_olp * batch_size # 5 * 10
    # print(train_labels)
    # pdb.set_trace()
...
def cross_validation():
    ...
    # epidata = np.load("/home/wenhuicu/data_npz/PTE_Allconn_AAL.npz")
    # lesion_epi = np.load('/home/wenhuicu/data_npz/PTE_lesion_vols.npz')['lesion_vols']
    lesion_epi = np.load('/home/wenhuicu/data_npz/PTE_lesion_vols_93.npz')['lesion_vols']

    # lesion_epi[:, :7, :] = 0
    # lesion_epi[:, -3:-1, :] = 0

    for i in range(lesion_epi.shape[1]):
        lesion_epi[:, i, :] = normalize(lesion_epi[:, i, :])
    # lesion_epi
    print(lesion_epi.shape)
    ...
    features_epi = torch.from_numpy(np.concatenate([epidata['features'], lesion_epi[:36]], axis=-1)).float().to(device) # n_subjectsx16x171
    ...
    # nonepidata = np.load("/home/wenhuicu/data_npz/NONPTE_Allconn_AAL.npz")
    # lesion_non = np.load('/home/wenhuicu/data_npz/NONPTE_lesion_vols.npz')['lesion_vols']
    lesion_non = np.load('/home/wenhuicu/data_npz/NONPTE_lesion_vols_93.npz')['lesion_vols']

    # lesion_non[:, :7, :] = 0
    # lesion_non[:, -3:-1, :] = 0
    for i in range(lesion_non.shape[1]):
        lesion_non[:, i, :] = normalize(lesion_non[:, i, :])
    ...
    features_non = torch.from_numpy(np.concatenate([nonepidata['features'], lesion_non[:36]], axis=-1)).float().to(device)
    # print("DAD shape:")
    ...
    for i in range(iterations):
        ...
        kfold = StratifiedKFold(n_splits=args.nfold, shuffle=False)
