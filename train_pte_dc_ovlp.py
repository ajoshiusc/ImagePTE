...
def split(feature, adj, labels):
    features_all = torch.zeros(
        [feature.shape[0] * args.ngroup, feature.shape[1], feature.shape[-1] // args.ngroup]).float().to(device)
    adj_all = torch.zeros([feature.shape[0] * args.ngroup, adj.shape[1], adj.shape[-1]]).float().to(device)
    labels_all = torch.zeros([feature.shape[0] * args.ngroup]).long().to(device)
    adj = adj.repeat(args.ngroup, 1, 1)
    labels = labels.repeat(args.ngroup)
    gl = feature.shape[-1] // args.ngroup
    slide_len = gl // 2
    k = 0
    for i in range(feature.shape[0]):
        for j in range(args.ngroup):
            # if i >= features_all.shape[0]:
            #     break
            features_all[k, :, :] = feature[i, :, j * gl:(j + 1) * gl]
            gap = feature.shape[0]
...
def train(epoch, model, optimizer, scheduler, train_features, train_adj, train_labels):
    ...
    for i in range(num_batches):
        ...
        acc_train, _ = accuracy_dc(graph_output, labels_bc, args.ngroup, device)
        loss_train.backward()
        optimizer.step()
        scheduler.step()
...
def test(model, test_features, test_adj, test_labels):
    ...
    prob_max = torch.zeros([probabilities.shape[0] // args.ngroup, 2]).float()
    k = 0
    for i in range(0, probabilities.shape[0], args.ngroup):
        prob_max[k, -1] = torch.mean(probabilities[i: i + args.ngroup, -1])
        prob_max[k, 0] = probabilities[i, 0]
        k = k + 1

    acc_test, test_labels_squeezed = accuracy_dc(graph_output, test_labels, args.ngroup, device)
